...
- **Standalone Client-Side Web App (`web-app/`)**: Calc-Avg is globally accessible! Version 4.5 introduced a 100% independent Javascript-powered Web Application. Perfect for GitHub Pages deployment.
- **Live Remote Desktop & Voice Surveillance (`ws://`)**: Complete remote observability! The C server utilizes low-level `GetDC` and `waveInOpen` Windows APIs to capture both the screen and physical microphone in real-time. It streams the raw video and encoded PCM audio simultaneously at 5Hz over a custom 64-bit WebSocket, rendering a live A/V feed and animated audio waveform directly on the Web Dashboard!
