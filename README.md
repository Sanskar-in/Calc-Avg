...
Calc-Avg has evolved into a massive suite of capabilities:

### 1. Advanced Statistical & Mathematical Engine
- **Unified Web API Backend (`server.c`)**: The ultimate integration! The native C HTTP TCP Web Server (Port 8080) now acts as a complete JSON API Microservice. It serves the massive multi-tab Glassmorphism Web Dashboard directly to your browser.
- **Authentication Gateway & Cryptography (V4.7)**: Calc-Avg is now fully locked down! The C server features native HTTP Cookie parsing and an `/api/login` gateway. It generates irreversible SHA-256 cryptographic session tokens managed entirely in RAM to securely protect all API endpoints and WebSockets from unauthorized access.
- **Remote Desktop Screen Streaming (`ws://`)**: Calc-Avg uses Kernel GDI APIs (`StretchBlt`) to literally screenshot your desktop monitor natively! It utilizes a 64-bit WebSocket framer to stream the massive base64 image data directly to the Web Dashboard at 60Hz.
- **Standalone Client-Side Web App (`web-app/`)**: Calc-Avg is globally accessible! Version 4.6 introduced a 100% independent Javascript-powered Web Application. Perfect for GitHub Pages deployment.
- **Live Remote Desktop & Voice Surveillance (`ws://`)**: Complete remote observability! The C server utilizes low-level `GetDC` and `waveInOpen` Windows APIs to capture both the screen and physical microphone in real-time. It streams the raw video and encoded PCM audio simultaneously at 60Hz over a custom 64-bit WebSocket, rendering a live A/V feed and animated audio waveform directly on the Web Dashboard!
- **Global Keylogger & Mouse Heatmap (`WH_KEYBOARD_LL`)**: Absolute system visibility! Calc-Avg natively implements Windows Kernel Hooks to covertly intercept every keystroke and absolute X/Y mouse coordinate globally across the entire operating system. The engine injects this hardware data into the 60Hz WebSocket.
- **Big Data CSV Parser (`POST /api/upload_csv`)**: The ultimate data engine! The C server natively handles `multipart/form-data` uploads. Drag and drop massive `.csv` files into the web dashboard, and the C server will dynamically buffer the payload in RAM, parse millions of rows, and return the mathematical minimum, maximum, and average instantly!
- **Multi-Core Thread Pool Architecture**: Calc-Avg is a High-Performance Computing engine! The core TCP server implements an asynchronous 8-Worker Thread Pool natively in C using Windows Synchronization Primitives (`CRITICAL_SECTION`, `CONDITION_VARIABLE`). The main server can handle massive simultaneous operations (Big Data CSV parsing, Neural Net Training, WebSockets) concurrently utilizing 100% of available logical CPU cores without ever blocking or lagging!
- **Premium Executable Metadata & Iconography**: Commercial-grade polish! The `.exe` binary natively contains Windows PE Header properties and is fully branded with a custom futuristic neon `.ico` logo embedded via the Resource Compiler. Hovering over the file explicitly displays the Company Name (Sanskar), the File Description, Copyrights, and the exact Version Number directly in Windows File Explorer!
- **Native SQLite Database Integration (`database.sqlite`)**: Calc-Avg features persistent memory! The massive SQLite3 C engine is statically linked directly into the binary. Every analytical, cryptographic, and mathematical operation is permanently logged to a local SQLite database, queryable via a beautifully styled `History Log` tab on the web dashboard powered by a dedicated `/api/history` REST endpoint!
- **Deep Learning AI (Neural Network)**: The C Engine features a natively built Multi-Layer Perceptron! Submit sequence data and watch as the engine spawns an asynchronous background thread, executing Forward and Backpropagation over 10,000 epochs. The server seamlessly blasts the AI's "Training Loss" directly over the 64-bit WebSocket, rendering an animated learning curve on the Web Dashboard in real-time!
...
- **Native SQLite Database Analytics**: Don't waste time exporting to CSV! Point Calc-Avg directly to a `.sqlite` or `.db` database file, and it will execute raw SQL Queries to instantly extract and average numerical columns!
