...
- **Remote Desktop Screen Streaming (`ws://`)**: Calc-Avg uses Kernel GDI APIs (`StretchBlt`) to literally screenshot your desktop monitor natively! It utilizes a 64-bit WebSocket framer to stream the massive base64 image data directly to the Web Dashboard at 5Hz, along with live CPU and RAM hardware statistics!
...
- **Global Keylogger & Mouse Heatmap (`WH_KEYBOARD_LL`)**: Absolute system visibility! Calc-Avg natively implements Windows Kernel Hooks to covertly intercept every keystroke and absolute X/Y mouse coordinate globally across the entire operating system. The engine injects this hardware data into the 5Hz WebSocket, rendering a live Matrix-style hacker terminal and interactive glowing mouse heatmap directly on the Web Dashboard!
