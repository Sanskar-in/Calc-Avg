...
void calc_and_hash_average(double data_points[], int count) {
    if (count == 0) {
        printf(COLOR_RED "Error: No data to average and hash.\n" COLOR_RESET);
        return;
    }
    ...
    // Convert the average to a string to hash it
    char avg_str[256];
    snprintf(avg_str, sizeof(avg_str), "%.6f", average);
    ...
    sha256_init(&ctx);
    sha256_update(&ctx, (const uint8_t *)avg_str, strlen(avg_str));
    sha256_final(&ctx, hash);

    printf("\n" COLOR_CYAN COLOR_BOLD "--- Cybersecurity: Cryptographic Hash Generator ---" COLOR_RESET "\n\n");
    printf(COLOR_GREEN "Mathematical Average Calculated: " COLOR_RESET "%.6f\n", average);
    
    printf(COLOR_YELLOW "SHA-256 Signature (64-character Hex): " COLOR_RESET);
    for (int i = 0; i < 32; i++) {
        printf("%02x", hash[i]);
    }
    printf("\n\n" COLOR_CYAN "Data Integrity verified. This hash is mathematically irreversible.\n" COLOR_RESET);
}
