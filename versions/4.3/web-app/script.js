...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const wsYval = document.getElementById('ws-yval');
    ...
    function initStreamChart() {
        ...
        streamChart = new Chart(ctx, {
            ...
            data: {
                labels: [],
                datasets: [{
                    label: 'Live Native C Data Stream (ws://)',
                    data: [],
                    borderColor: '#f43f5e',
            ...
            options: {
                ...
                scales: { 
                    y: { grid: { color: 'rgba(255,255,255,0.05)' }, suggestedMin: -150, suggestedMax: 150 },
                    x: { display: false }
                }
            }
        });
    ...
    startBtn.addEventListener('click', () => {
        ...
        ws.onopen = () => {
            wsStatus.innerText = "Connected (20Hz Live)";
        ...
        ws.onmessage = (event) => {
            try {
                ...
                wsYval.innerText = data.y.toFixed(2);
                
                // Add to chart
                streamChart.data.labels.push(data.x.toFixed(2));
                streamChart.data.datasets[0].data.push(data.y);
                
                // Keep only the last 50 points so it scrolls
